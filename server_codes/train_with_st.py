...
from ResUnet import ResUnet, ResUnetST
from tqdm import *
...
def train(loss_func, train_with_st):
    epochs = 1000
    bs = 32
    lr = 0.001
    weight_decay = 1e-4
    ...
    # train_loader = DataLoader(train_dataset, batch_size=bs, shuffle=True, num_workers=16, drop_last=True)
    #
    ...
    model = ResUnetST(3, 12, 1) if train_with_st else ResUnet(3, 1)
    ...
    save_dir_path = os.path.join('Experiments', f'{"COMBINED" if train_with_st else "SATONLY"}_RESUNET_BS{bs}_LR{lr}_{loss_func.upper()}')
    ...
    #         logits = model(sat_im, street_ims) if train_with_st else model(sat_im)
    ...
    # print('testing...')
    # test(save_dir_path, train_with_st)
    # print('testing over')
    # print('--------------------------------------------------------------------')
...
if __name__ == "__main__":
    ...
    train(loss_func='sie', train_with_st=False)
